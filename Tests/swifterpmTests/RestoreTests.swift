...
struct RestoreTests {
    ...
    func restorePackageWithNoPinsUsesScopedScratchAndCache() async throws {
        try await withTemporaryDirectory { root in
            ...
            #expect(try await AsyncFileSystem.exists(scratch.appendingPathComponent("registry/downloads")))
        }
    }
    ...
    func writeWorkspaceStateWritesSourceControlAndRegistryDependencies() async throws {
        try await withTemporaryDirectory { root in
            ...
            try await writeWorkspaceState(packageDir: package, scratchDir: scratch, resolved: resolved, disableSandbox: false)

            let statePath = scratch.appendingPathComponent("workspace-state.json")
            let state = try #require(JSONSerialization.jsonObject(with: try await AsyncFileSystem.readData(from: statePath)) as? [String: Any])
